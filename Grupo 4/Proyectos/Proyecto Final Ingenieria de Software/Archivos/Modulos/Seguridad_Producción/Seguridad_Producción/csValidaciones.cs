        ...
        public static int Validar_Usuario(string vUsuario, string vContrasena, string vEmpresa)
        {
            try
            {
               ...
               // string sValidar = "SELECT * FROM tabm_usuario WHERE user_usu='" + vUsuario + "' AND pass_usu='" + vContrasena + "';";
                //string sValidar2 = "select user_usu, pass_usu, cod_empsa from tabm_usuario where user_usu = '"+ vUsuario +"' and pass_usu = '"+ vContrasena +"' and cod_emp = '"+ vEmpresa +"'; ";
                string sValidar2 = "select user_usu, pass_usu, cod_empsa from tabm_usuario inner join tabm_empleado on tabm_empleado.cod_emp = tabm_usuario.cod_emp where user_usu = '" + vUsuario + "' and pass_usu = '" + vContrasena + "' and tabm_empleado.cod_empsa = '" + vEmpresa + "';";
